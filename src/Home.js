      ...
      <div style={contentWrapper}>
        ...
        <div style={{ flex: 1, paddingTop: '120px' }}>
          ...
          <section style={{ textAlign: 'center', padding: '60px 20px' }}>
            ...
            <button style={{ ...registerButton, fontSize: '1.5rem', padding: '15px 40px', marginTop: '20px' }} onClick={() => setShowRegisterModal(true)}>
              立即註冊 ➜
            </button>
          </section>
        </div>
          ...
          <div style={modalOverlayStyle} onClick={() => setShowRegisterModal(false)}>
            <div style={modalContentStyle} onClick={e => e.stopPropagation()}>
              ...
              <div style={{ display: 'flex', justifyContent: 'center', margin: '10px 0' }}>
                <input type="text" placeholder="驗證碼" value={regCode} onChange={(e) => setRegCode(e.target.value)} style={{ width: '50%', padding: '8px', marginRight: '10px' }} />
                <button onClick={sendVerificationCode} style={{ ...modalRegisterButton, padding: '8px 12px' }}>發送驗證碼</button>
              </div>
              {regErrorMsg && <p style={{ color: 'red' }}>{regErrorMsg}</p>}
              {regSuccessMsg && <p style={{ color: 'green', fontWeight: 'bold' }}>{regSuccessMsg}</p>}
              <button style={modalRegisterButton} onClick={handleRegister}>提交</button>
            </div>
          </div>
        )}
